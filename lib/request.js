...
module.exports = {
  ...
  login: function (sessionConfig, cb) {
    ...
    if (!options.certFile || !options.keyFile) {
      cfg.url = 'https://identitysso.betfair.it/api/login';
    } else {
      cfg.agentOptions = {
  ...
  send: function (session, params, cb) {
    ...
    var optionsRequest = {
      url: 'https://api.betfair.it/exchange/betting/rest/v1.0/' + action + '/',
      json: data,
      headers: {
  ...
  sendAccount: function (session, params, cb) {
    ...
    var optionsRequest = {
      url: 'https://api.betfair.it/exchange/account/rest/v1.0/' + action + '/',
      json: data || true,
      headers: {
        'X-Application': session.applicationKey,
        'X-Authentication': session.token,
  ...
  heartbeat: function (session, params, cb) {
    ...
    var optionsRequest = {
      url: 'https://api.betfair.it/exchange/heartbeat/json-rpc/v1',
      json: {
        "params": params,
        "jsonrpc": "2.0",
  ...
  sendRPC: function (session, params, cb) {
    ...
    var optionsRequest = {
      url: 'https://api.betfair.it/exchange/betting/json-rpc/v1',
      json: params,
      headers: {
        'X-Application': session.applicationKey,
        'X-Authentication': session.token,
